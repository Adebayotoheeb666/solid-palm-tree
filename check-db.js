...
async function checkDatabaseConnectivity() {
  ...
  try {
    ...
    console.log(`   Status: ${result.supabase.connected ? '✅ Connected' : '❌ Disconnected'}`);
    ...
    console.log(`   Details:`, JSON.stringify(result.supabase.details, null, 4));
    console.log('');
    
    // Fallback Status
    console.log('🔄 FALLBACK SYSTEM:');
    console.log(`   Status: ${result.fallbackData.connected ? '✅ Available' : '❌ Unavailable'}`);
    ...
    console.log(`   Details:`, JSON.stringify(result.fallbackData.details, null, 4));
    console.log('');
    
    // Overall Status
    console.log('📈 OVERALL SYSTEM HEALTH:');
    const statusEmoji = result.overall.status === 'healthy' ? '✅' : 
                       result.overall.status === 'degraded' ? '⚠️' : '❌';
    console.log(`   Status: ${statusEmoji} ${result.overall.status.toUpperCase()}`);
