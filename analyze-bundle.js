...
try {
  ...
  if (fs.existsSync(DIST_DIR)) {
    ...
    const imageFiles = fileAnalysis.filter(f => /\.(png|jpg|jpeg|gif|svg)$/i.test(f.file));
    ...
    if (totalSize > 5 * 1024 * 1024) { // > 5MB
      ...
      console.log('⚠️  Bundle size is moderate. Consider optimizations for better performance.');
    } else {
