...
class ServiceLogger {
  ...
  private async checkAuth(): Promise<{ status: 'working' | 'error'; message?: string }> {
    try {
      ...
      if (token) {
        ...
        if (response.ok) {
          return { status: 'working', message: 'Authentication service operational' };
        }
      }
      return { status: 'working', message: 'Authentication service ready (not logged in)' };
    } catch (error) {
      return { status: 'error', message: `Auth check failed: ${error}` };
    }
  }

  private async checkDatabase(): Promise<{ status: 'working' | 'error'; message?: string }> {
    try {
  ...
  private async checkEmailService(): Promise<{ status: 'working' | 'error'; message?: string }> {
    try {
      // Check if email service is configured by examining environment
      const isConfigured = document.cookie.includes('email_configured=true') || 
                          window.location.hostname !== 'localhost';
      
      if (isConfigured) {
        return { status: 'working', message: 'Email service configured' };
      }
      return { status: 'error', message: 'Email service not configured (SendGrid)' };
    } catch (error) {
      return { status: 'error', message: `Email service check failed: ${error}` };
    }
  }

  private async checkStripe(): Promise<{ status: 'working' | 'error'; message?: string }> {
    try {
  ...
  private async checkAmadeus(): Promise<{ status: 'working' | 'error'; message?: string }> {
    try {
      const response = await fetch('/api/amadeus/airports/search?q=NYC', {
        method: 'GET'
      });
      if (response.ok) {
        return { status: 'working', message: 'Amadeus API service operational' };
      }
  ...
  private async checkSupabase(): Promise<{ status: 'working' | 'error'; message?: string }> {
    try {
      ...
      if (!error && data) {
        return { status: 'working', message: `Supabase operational (${data.length} airports loaded)` };
      }
  ...
  private async checkService(
    serviceName: string, 
    checkFunction: () => Promise<{ status: 'working' | 'error'; message?: string }>
  ) {
    try {
  ...
  private updateServiceStatus(name: string, status: 'working' | 'error' | 'unknown', message?: string) {
  ...
  public logServiceStatus() {
    ...
    if (serviceList.length === 0) {
      ...
      serviceList.forEach(service => {
        const emoji = service.status === 'working' ? '✅' : 
                     service.status === 'error' ? '❌' : '⚠️';
        const timeAgo = this.getTimeAgo(service.timestamp);
        
        console.log(`${emoji} ${service.name}: ${service.status.toUpperCase()} (${timeAgo})`);
        if (service.message) {
      ...
      console.log('-----------------------------------');
      console.log(`📊 Summary: ${working}/${total} services working, ${errors} errors`);
