...
const sendBookingConfirmationEmail = async (booking: any) => {
  try {
    ...
    try {
      const response = await fetch(`${process.env.CLIENT_URL || 'http://localhost:8080'}/api/email/booking-confirmation`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.error("❌ Failed to send booking confirmation email:", response.status);
      }
    } catch (error) {
    ...
    switch (event.type) {
      ...
      case "payment_intent.requires_action": {
        ...
        if (booking) {
          ...
          console.log(
            `Booking ${booking.id} marked as pending - requires action`,
          );
        }
        break;
      }
