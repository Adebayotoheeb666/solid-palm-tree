...
export async function handleCreateGuestBooking(req: Request, res: Response) {
  try {
    ...
    const { data: booking, error: bookingError } = await supabaseServerHelpers.createGuestBooking({
...
export async function handleGetGuestBooking(req: Request, res: Response) {
  try {
    ...
    const { data: booking, error } = await supabaseServerHelpers.getGuestBookingByPNR(pnr as string, email as string);
    ...
    const { data: fromAirport } = await supabaseServerHelpers.getAirportById(booking.from_airport_id);
    const { data: toAirport } = await supabaseServerHelpers.getAirportById(booking.to_airport_id);

    // Get passengers
    const { data: passengers } = await supabaseServerHelpers.getPassengersByBookingId(booking.id);
